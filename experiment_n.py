...
def main():
    ...
    #Run Iteration
    for i in range(1,6):
        os.system(f"python ./EA_n.py -o Run_{i} -e 2-4-6-8")
        champ = np.load(f'EA_n/enemy_2-4-6-8/Run_'+str(i)+'/best_genome.npy')
        champs.append(champ)
    ...
    for j,c  in enumerate(champs):
        ...
        with open(f'EA_n/enemy_2-4-6-8/Run_'+str(j+1)+'/best_5_f.csv','w') as f:
            writer = csv.writer(f)
