...
import random
import numpy as np
from Chrommm import Population
from Chrommm import Chrom
...
def testing_pop(pop):
    for i in range (len(pop.chrom_list)):
        pop.chrom_list[i].fitness = evaluate(pop.chrom_list[i])
    
###we could creat 1 method instead of 2 up here!
...
def reproduction(parents):
    ...
    for j in range(int(n_offspring/(n_parents/2))): #based on how many offpring we want: 400? and the pop is 100? then it will iterate 8 times #hopefully they will not be the same bc of mutation
        for i in range(0, parents.get_size(), 2):
            ...
            new_genome = crossover(p1, p2)
            offspring.add_chroms(Chrom(new_genome))
...
def main():
    ...
    pop = Population(pop_size,chrom_size)
    testing_pop(pop)
    pop.sort_by_fitness()
